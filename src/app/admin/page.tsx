...
export default function AdminDashboardPage() {
  ...
  // Load persisted configs and MongoDB/Supabase state on mount
  useEffect(() => {
    // 1. Trigger Seed if empty
  ...
  // Save Customizer Config (Sync with LocalStorage & Database API)
  const handleSaveCustomizer = () => {
    ...
    fetch('/api/settings', {
      ...
      .then((res) => res.json())
      .then(() => {
        showToast('Saved to Database & LocalStorage Successfully!');
      })
      .catch(() => {
  ...
  const handleSaveBlogPost = (e: React.FormEvent) => {
    ...
    fetch('/api/blogs', {
      ...
      .then((data) => {
        if (data.success) {
          if (editingPost) {
            setBlogPosts(blogPosts.map((p) => (p.id === editingPost.id ? { ...p, ...blogForm } : p)));
          } else {
            setBlogPosts([data.post || { id: Date.now(), ...blogForm, date: '2026-07-29' }, ...blogPosts]);
          }
          showToast(`Saved blog article "${blogForm.title}"!`);
        }
      })
      .catch(() => {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
            ...
            <div className="lg:col-span-6 space-y-6">
              <GlassCard hoverEffect={false} className="space-y-4 border-[#00FF99]/40">
                ...
                <div className="space-y-3 text-xs">
                  ...
                  <div>
                    <label className="block text-slate-300 font-bold mb-1">Engineering Hotline / Phone</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-slate-300 font-bold mb-1">Global Headquarters Address</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-slate-300 font-bold mb-1">Map Center Location Text</label>
                    <input
                      type="text"
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-8">
          <div className="flex items-center justify-between glass-panel p-4 rounded-2xl border border-slate-800">
            ...
            >
              <RotateCcw className="w-4 h-4" /> Reset Analytics (Fresh 0 Baseline)
            </button>
          </div>
      ...
      {activeTab === 'blogs' && (
        <GlassCard hoverEffect={false} className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-bold text-white flex items-center gap-2">
              <Newspaper className="w-5 h-5 text-[#00E5FF]" /> Blog & Industry News CMS ({blogPosts.length} Articles)
            </h3>
              ...
              <tbody className="divide-y divide-slate-800">
                {blogPosts.map((b: any) => (
                  <tr key={b.id}>
                    ...
                    <td className="p-3 flex items-center gap-2">
                      <button onClick={() => handleOpenBlogModal(b)} className="p-1 text-slate-300 hover:text-[#00FF99]">
                        <Edit className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => handleDeleteBlogPost(b.id)} className="p-1 text-red-400 hover:text-red-300">
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    </td>
                  </tr>
      ...
      {activeTab === 'media' && (
        <GlassCard hoverEffect={false} className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setMediaModalOpen(true)} className="px-4 py-2 rounded-xl bg-pink-600 text-white text-xs font-bold flex items-center gap-1">
              <Plus className="w-4 h-4" /> Upload Asset
            </button>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {mediaItems.map((m) => (
              <div key={m.id} className="glass-panel p-3 rounded-xl space-y-2">
                <div className="text-xs font-mono text-[#00FF99] truncate">{m.name}</div>
                <div className="text-[10px] text-slate-400">{m.size}</div>
                <button onClick={() => handleDeleteMedia(m.id)} className="text-xs text-red-400 flex items-center gap-1">
      ...
      {activeTab === 'messages' && (
        <GlassCard hoverEffect={false} className="space-y-6">
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            <Mail className="w-5 h-5 text-[#00FF99]" /> Contact Messages ({messages.length})
          </h3>
          <div className="space-y-3">
            {messages.map((m) => (
              <div key={m.id} className="glass-panel p-4 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  <div className="text-xs font-bold text-white">{m.name} ({m.email})</div>
                  <button onClick={() => handleOpenReplyModal(m)} className="px-3 py-1 rounded-lg bg-[#00FF99] text-black text-xs font-bold flex items-center gap-1">
                    <Send className="w-3 h-3" /> Reply Email
                  </button>
                </div>
                <div className="text-xs text-slate-300 font-semibold">{m.subject}</div>
                <div className="text-xs text-slate-400">{m.body}</div>
              </div>
            ))}
          </div>
        </GlassCard>
      ...
      {activeTab === 'logs' && (
        <GlassCard hoverEffect={false} className="space-y-6">
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            <Lock className="w-5 h-5 text-emerald-400" /> Security & Audit Logs ({securityLogs.length})
          </h3>
          <div className="space-y-2 text-xs font-mono">
            {securityLogs.map((l) => (
              <div key={l.id} className="p-3 rounded-xl bg-slate-950 flex items-center justify-between">
      ...
      {activeTab === 'users' && (
        <GlassCard hoverEffect={false} className="space-y-6">
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            <Users className="w-5 h-5 text-[#00E5FF]" /> User Directory ({users.length})
          </h3>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-950/80 text-slate-400">
              ...
              <tbody className="divide-y divide-slate-800">
                {users.map((u) => (
                  <tr key={u.id}>
                    ...
                    <td className="p-3">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${u.status === 'Active' ? 'bg-emerald-500/20 text-[#00FF99]' : 'bg-red-500/20 text-red-400'}`}>
                        {u.status}
                      </span>
                    </td>
                    <td className="p-3 flex items-center gap-2">
                      <button onClick={() => toggleUserBlock(u.id)} className="text-slate-300 hover:text-white">
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-6 space-y-6">
              ...
              <GlassCard hoverEffect={false} className="space-y-4">
                <h3 className="text-base font-bold text-[#00E5FF] flex items-center gap-2">
                  <Settings className="w-5 h-5" /> Infrastructure & Cloud Database Config
                </h3>
                <div className="space-y-3 text-xs">
                  <div>
                    <label className="block text-slate-400 font-mono mb-1">ACTIVE SUPABASE PROJECT URL</label>
                    <input
                      type="text"
                      readOnly
                      value="https://txfxinxhqmeopjscanyz.supabase.co"
                      className="w-full px-3.5 py-2 rounded-xl glass-panel text-emerald-400 font-mono"
                    />
                  </div>
                  <div>
                    <label className="block text-slate-400 font-mono mb-1">PUBLISHABLE API KEY</label>
                    <input
                      type="password"
                      readOnly
                      value="sb_publishable_Iu_k8_IGal5pK8dGET3p4w_lAv95L_X"
                      className="w-full px-3.5 py-2 rounded-xl glass-panel text-white font-mono"
                    />
                  </div>
                  <a
                    href="https://supabase.com/dashboard/project/txfxinxhqmeopjscanyz"
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1.5 text-xs text-[#00FF99] font-bold underline pt-1"
                  >
                    Open Supabase Cloud Dashboard <ExternalLink className="w-3.5 h-3.5" />
                  </a>
                </div>
              </GlassCard>
            </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        ...
                        onChange={(e) => handleSave2FA({ ...twoFactorConfig, totpEnabled: e.target.checked })}
      ...
      <GlassModal isOpen={showQrModal} onClose={() => setShowQrModal(false)} title="Setup Google Authenticator 2FA">
        <div className="text-center space-y-4 py-4 text-xs">
          <p className="text-slate-300">Scan this QR code with Google Authenticator app:</p>
